...
void MPU9250_DMP::computeEulerAngles(bool degrees)
{
    ...
    roll = atan2(gravity.y , gravity.z);
...
//add 6050 function
uint8_t MPU9250_DMP::dmpGetGravity(VectorFloat *v) {
	int32_t cqx,cqy,cqz,cqw;
	int16_t gqx,gqy,gqz,gqw;
	cqx = qx;
	cqy = qy;
	cqz = qz;
	cqw = qw;
    ...
    v -> x = 2 * (gqx*gqz - gqw*gqy);
    v -> y = 2 * (gqw*gqx + gqy*gqz);
    v -> z = gqw*gqw - gqx*gqx - gqy*gqy + gqz*gqz;
    return 0;
}